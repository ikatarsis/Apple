...
export const VideoCarousel = () => {
  ...
  const {isEnd, startPlay, videoId, isLatestVideo, isPlaying} = video;
  ...
  useEffect(() => {
    ...
    if(span[videoId]){
      const anim = gsap.to(span[videoId], {
        ...
        onComplete: () => {
          if(isPlaying){
            gsap.to(videoDivRef.current[videoRef], {
              width: '12px',
            });
  ...
  const handleProcess = (type, index) => {
    ...
    case 'video-end':
      ...
      break;
    case 'video-last':
      setVideo(prevVideo => ({...prevVideo, isLatestVideo: true}));
      break;
    case 'video-reset':
      setVideo(prevVideo => ({...prevVideo, isLatestVideo: false, videoId: 0}));
      break;
    case 'play':
    ...
    <>
            ...
            <div className='video-carousel_container'>
              <div className='w-full h-full flex flex-center rounded-3xl overflow-hidden bg-black'>
                <video id='video'
                  ...
                  onLoadedData={e => handleLoadedMetadata(index, e)}
                >
      ...
      <div className='relative flex flex-center mt-10'>
        ...
        <button className='control-btn'>
          <img src={isLatestVideo ? replayImg : !isPlaying ? playImg : pauseImg}
            alt={isLatestVideo ? 'replay' : !isPlaying ? 'play' : 'pause'}
            onClick={isLatestVideo ? () => handleProcess('video-reset') : !isPlaying ? () => handleProcess('play') : () => handleProcess('pause')}
          />
        </button>
      </div>
    </>
